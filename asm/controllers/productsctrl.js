...
$scope.$watchGroup(['mau', 'keyword'], function(newValues) {
    ...
    $scope.dsSPLocTheoMau = $scope.dsSP.filter(function(sp) {
    ...
    if (newKeyword) {
        $scope.dsSPLocTheoMau = $scope.dsSPLocTheoMau.filter(function(sp) {
    ...
    $scope.dsSPHienThi = $scope.dsSPLocTheoMau;

    // Lưu lại bản sao khi áp dụng bộ lọc màu và từ khóa
    $scope.dsSPHienThiGoc = angular.copy($scope.dsSPHienThi);

    // Reset lại trang về 1 khi thay đổi màu hoặc tên
    $scope.page = 1;
    $scope.start = 0;
    $scope.tongTrang = Math.ceil($scope.dsSPHienThi.length / $scope.limit);
...
$scope.$watch('sapxep', function(newSapXep) {
    ...
    if (newSapXep === 'tangDan') {
        ...
        $scope.dsSPHienThi = $scope.dsSPHienThi.sort(function(a, b) {
            return a.Price - b.Price;
        });
        console.log($scope.dsSPHienThi);
        ...
        $scope.dsSPHienThi = angular.copy($scope.dsSPHienThiGoc);
        console.log($scope.dsSPHienThi);
    }
});

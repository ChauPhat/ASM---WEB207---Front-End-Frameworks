app.controller('products-bags-ctrl', function ($scope, $rootScope, $http) {

    // lấy sản phẩm từ dsSP có type là top
    $scope.dsSPBAGS = [];

    for (var i = 0; i < $scope.dsSP.length; i++) {
        if ($scope.dsSP[i].Type == 'Bags') {
            $scope.dsSPBAGS.push($scope.dsSP[i]);
            // console.log($scope.dsSPBAGS);
        }
    }
    ...
    $scope.tongTrang = Math.ceil($scope.dsSPBAGS.length / $scope.limit);
...
$scope.$watchGroup(['mau', 'keyword'], function(newValues) {
    ...
    $scope.dsSPBAGSLocTheoMau = $scope.dsSPBAGS.filter(function(sp) {
    ...
    if (newKeyword) {
        $scope.dsSPBAGSLocTheoMau = $scope.dsSPBAGSLocTheoMau.filter(function(sp) {
    ...
    $scope.dsSPBAGSHienThi = $scope.dsSPBAGSLocTheoMau;

    // Lưu lại bản sao khi áp dụng bộ lọc màu và từ khóa
    $scope.dsSPBAGSHienThiGoc = angular.copy($scope.dsSPBAGSHienThi);

    // Reset lại trang về 1 khi thay đổi màu hoặc tên
    $scope.page = 1;
    $scope.start = 0;
    $scope.tongTrang = Math.ceil($scope.dsSPBAGSHienThi.length / $scope.limit);
...
$scope.$watch('sapxep', function(newSapXep) {
    ...
    if (newSapXep === 'tangDan') {
        ...
        $scope.dsSPBAGSHienThi = $scope.dsSPBAGSHienThi.sort(function(a, b) {
            return a.Price - b.Price;
        });
        console.log($scope.dsSPBAGSHienThi);
        ...
        $scope.dsSPBAGSHienThi = angular.copy($scope.dsSPBAGSHienThiGoc);
        console.log($scope.dsSPBAGSHienThi);
    }
});
